...
Pages: / (home)  /detect  /history  /analytics  /about
Run:  python app.py
...
def load_predictor():
    ...
    try:
        ...
        model.load_state_dict(ckpt["state"])
        model.eval()
        _predictor = {"model": model, "vocab": vocab, "val_f1": ckpt.get("val_f1", 0)}
...
def predict(body=""):
    ...
    spam = float(probs[1]); ham = float(probs[0])
    return {"label": "spam" if spam>=0.5 else "ham",
            "spam_score": round(spam,4), "ham_score": round(ham,4),
            "confidence": round(max(spam,ham),4)}

def model_info():
    if _predictor is None: return {}
    m = _predictor["model"]
    return {"architecture":"BiLSTM",
...
def init_db():
    with sqlite3.connect(DB) as db:
        db.execute("""CREATE TABLE IF NOT EXISTS predictions (
            ...
            spam_score REAL, ham_score REAL, confidence REAL,
...
@app.route("/detect")
def page_detect():   return render_template("detect.html")
...
def route_history():
    ...
    rows  = db.execute("SELECT * " + base + " ORDER BY created_at DESC LIMIT ? OFFSET ?",
...
def route_stats():
    ...
    return jsonify({"total": total, "spam": spam, "ham": total-spam,
